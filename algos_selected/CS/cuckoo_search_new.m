...
function [res]=cuckoo_search_new(D,fhd,lb,ub,maxit,popsize)
...
N_IterTotal=maxit/2; %evaluation twice in loop
...
RecordFEsFactor = round(logspace(log(2*n)/log(10),log(maxit*popsize)/log(10),nr_points_to_save));
Run_RecordFEsFactor=RecordFEsFactor;
run_funcvals = []; run_funccalls = [];
...
[fmin,bestnest,nest,fitness]=get_best_nest(nest,nest,fitness,fhd);
...
for iter=1:N_IterTotal,
    % Generate new solutions (but keep the current best)
     ...
     [fnew,best,nest,fitness]=get_best_nest(nest,new_nest,fitness,fhd);
    % Update the counter
      N_iter=N_iter+n; 
    % Discovery and randomization
      new_nest=empty_nests(nest,Lb,Ub,pa) ;
    
    run_funcvals = [run_funcvals; fmin];
        run_funccalls = [run_funccalls; N_iter];
...
run_funcvals(end) = fmin;
res.solution = bestnest;
res.bestval = fmin;
...
function [fmin,best,nest,fitness]=get_best_nest(nest,newnest,fitness,fhd)
% Evaluating all new solutions
parfor j=1:size(nest,1),
    fnew=fobj(newnest(j,:),fhd);
    if fnew<=fitness(j),
       fitness(j)=fnew;
       nest(j,:)=newnest(j,:);
    end
